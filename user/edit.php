...
if ($_POST['but'] == 'save') {
    ...
    $us = new user($user_id);
    $res_usr = $us->editUser($_POST['emp_name']);
    $us->getUser($user_id);
    $emp = new employee($us->employeeID);
    //$lat = $_POST['lat'];
    //$lon = $_POST['lon'];
    $res_emp = $emp->editEmployee($_POST['emp_name'], $_POST['emp_address'], $_POST['emp_nic'], $_POST['emp_mobile'], $_POST['emp_email'], $_POST['emp_designation'], '', $_POST['note']);
    if ($_POST['usr_psw'] != '') {
...
if ($_POST['submit_value'] == 'delete') {
    ...
    $us->getUser($user_id);
    $emp = new employee();
    $res_emp = $emp->deleteEmployee($us->employeeI);
    $res_usr = $us->deleteUser($user_id);
    //print $res_usr; // NEED to remove
    if ($res_usr) {
        $close = true;
    } else {
        $close = false;
                    ...
                    <section class="col-xs-3 col-sm-3 col-md-3">
                     ...
                     <section class="col-xs-7 col-sm-7 col-md-7">
                       ...
                       <input type="hidden" name="UID" id="UID" value="<?php print $us->id ?>"> 
                        <input type="hidden" name="EMPID" id="EMPID" value="<?php print $us->employeeID ?>">
                    
                <footer> 
                    <?php /* if($us->status == constants::$USER_DELETED) {
                            print '<p style="" id=""><font color="red"><i class="fa-fw fa fa-times"></i>Deleted User</font></p>';
                        } else { */?>
                    <button type="submit" class="btn btn-primary" name="but" id="but_save" value="save">
                        Save
                    </button>
                    <button type="submit" class="btn btn-primary" name="submit_value" id="but_delete" value="delete">
                        Delete
                    </button>
                </footer>          
<?php
//print_r($ary_prev); //Array ( [1] => Array ( [0] => 3 ) [2] => Array ( [0] => 5 ) ) 
/*if ($ary_prev[2][0] == '6') {
...
<?php
$prev = false;
if ($ary_prev[2][0] == '6') {//Save page view
    print '$( "input" ).prop( "disabled", false );';
    print '$( "#but" ).show();';
    print '$( "textarea" ).prop( "disabled", false );';
    $prev = true;
}
if ($ary_prev[2][0] == '6') {//delete page view
    print '$( "input" ).prop( "disabled", false );';
